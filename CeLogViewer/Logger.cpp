...
//// Definitions
#define LOG_PATH     _T("\\Temp")
#define DEFAULT_NAME _T("CeLogViewer")
...
bool Logger::Open(LPTSTR psLogFile)
{
	...
	// Open file.
	m_pFile = _tfopen(psLogFile, _T("at")); // ANSI
	//m_pFile = _tfopen(psLogFile, _T("wt")); // ANSI
	//m_pFile = _tfopen(psLogFile, _T("wt, ccs=UTF-8")); // Unicode support if later needed.
...
bool Logger::WriteTimestamp()
{
	...
	if (_stprintf(psTimestamp, _T("%u-%02u-%02u-%02u%02u-%02u\0"), Time.wYear,
																  Time.wMonth,
																  Time.wDay,
																  Time.wHour,
																  Time.wMinute,

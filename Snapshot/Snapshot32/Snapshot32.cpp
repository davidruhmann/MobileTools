...
bool ProcessSnapshot      (PROCESSENTRY32* pProcessEntry, bool bImage = false);
...
bool SnapshotProcesses    (TCHAR* psTarget = NULL, bool bImage = false);
bool SnapshotThreads      (DWORD nPID);
bool SnapshotWindows      (DWORD nTargetPID, DWORD nLevel = (DWORD)0, HWND hWnd = NULL, bool bImage = false); // Requires MFC.
...
int _tmain(int argc, TCHAR* argv[], TCHAR* envp[])
{
	...
	// TODO Make Image folder same as log folder.
	// l  Log File. (Default C:\Temp)
	// t  Target Process. (Default all)
	// e  Image Encoder Command.
	// i  Export Images.
	// - or /  Escape Command Characters.
	// e.g. Snapshot.exe -l "path\Logfile" -t "targetproc.exe" -i
	TCHAR c;
	for (int x=0; x < argc; ++x)
	{
		...
		if(c == _T('/') || c == _T('-'))
		{
			...
			case _T('l'): // Log File
				...
				{
					int o = 0;
					int n = _tcslen(argv[x+1]);

					// Check for surrounding quotations to remove.
					if (argv[x+1][0]   == _T('\"')) { --n; o = 1; }
					if (argv[x+1][n-1] == _T('\"')) { --n; }

					// Copy string.
					psLog = new TCHAR[n+1];
					_tcsncpy_s(psLog, n+1, argv[x+1] + o, n);
					_tcsncpy_s(psLog + n, 1, _T("\0"), 1);

					#ifdef DEBUG
						// Display Value.
						_tprintf_s(_T("Log:    %s"), psLog);
					#endif
				}
				break;
			case _T('t'): // Target Process
				// Next argument should be a string.
				if (x+1 <= argc)
				{
					int o = 0;
					...
					// Copy string.
					psTarget = new TCHAR[n+1];
					_tcsncpy_s(psTarget, n+1, argv[x+1] + o, n);
					_tcsncpy_s(psTarget + n, 1, _T("\0"), 1);

					#ifdef DEBUG
						// Display Value.
						_tprintf_s(_T("Target: %s"), psTarget);
					#endif
				}
				break;
			case _T('e'): // Encoder Command.
				...
				if (x+1 <= argc)
				{
					//nType = (EncoderType)StringToDWORD(argv[x+1]);
				}
				break;
			case _T('i'): // Generate Images.
				// This is a lone argument.
				bImage = true;
				#ifdef DEBUG
					// Display Value.
					_tprintf_s(_T("Image:  Enabled"));
				#endif
				break;
			}
		}
	...
	bool nResult = SnapshotProcesses(psTarget, bImage);
...
bool LogProcessEntry(PROCESSENTRY32* pProcessEntry)
{
	...
	_stprintf_s(psMessage, MAX_PATH, _T("Process:    %s\0"), pProcessEntry->szExeFile);
	pLog->WriteMessage(psMessage);
	_stprintf_s(psMessage, MAX_PATH, _T("ID:         0x%X\0"), pProcessEntry->th32ProcessID);
	pLog->WriteMessage(psMessage);
	_stprintf_s(psMessage, MAX_PATH, _T("Threads(#): %u\0"), pProcessEntry->cntThreads);
	pLog->WriteMessage(psMessage);
	_stprintf_s(psMessage, MAX_PATH, _T("Parent ID:  0x%X\0"), pProcessEntry->th32ParentProcessID);
	pLog->WriteMessage(psMessage);
	_stprintf_s(psMessage, MAX_PATH, _T("Heap ID:    0x%X\0"), pProcessEntry->th32DefaultHeapID);
	pLog->WriteMessage(psMessage);
	// Windows Mobile Only
...
bool ProcessSnapshot(PROCESSENTRY32* pProcessEntry, bool bImage /*= false*/)
{
	...
	SnapshotWindows(pProcessEntry->th32ProcessID, bImage);
...
bool SnapshotHeap(DWORD nPID)
{
	...
	do
	{
		...
		} while (Heap32Next(&HeapEntry) == TRUE);
		//// Heap Entry End.
	...
	// Write heap tally.
	_stprintf_s(psMessage, 48, _T("Heap Total = %u bytes\0"), nTotalSize);
	pLog->WriteMessage(psMessage);
...
bool SnapshotModules(DWORD nPID)
{
	...
	{
		// Log the module.
		_stprintf_s(psMessage, MAX_PATH+MAX_PATH+40, _T("0x%8X  %10u  %10u  %s\\%s\0"), ModuleEntry.th32ModuleID,
...
bool SnapshotProcesses(TCHAR* psTarget /*= NULL*/, bool bImage /*= false*/)
{
	...
	do
	{
			...
			(psTarget != NULL && _tcsicmp(ProcessEntry.szExeFile, psTarget) == 0))
		{
			...
			ProcessSnapshot(&ProcessEntry, bImage);
		}
...
bool SnapshotThreads(DWORD nPID)
{
	...
	pLog->WriteMessage(_T("\n\n -- Threads --"));
	pLog->WriteTimestamp();
	pLog->WriteMessage(_T("-- 0xID --  -- Owner --  -- nUse --"));
	...
	do
	{
		...
		// Log the thread.
		_stprintf_s(psMessage, 64, _T("0x%8X   0x%8X  %10u  \0"), ThreadEntry.th32ThreadID,
...
bool SnapshotWindows(DWORD nTargetPID, DWORD nLevel /*= 0*/, HWND hWnd /*= NULL*/, bool bImage /*= false*/)
{
	//bool bContinue(true);
		...
		if (nCurrentPID == nTargetPID)
		{
			...
			if (bImage)
			{
				_stprintf_s(psFilename, 256,
					_T("%s\\%u-%02u-%02u-%02u%02u-%02u-%08X_%s\0"), _T("C:\\Temp"),
																	 Time.wYear,
																	 Time.wMonth,
																	 Time.wDay,
																	 Time.wHour,
			...
			// Log the Window Info.
			_stprintf_s(psMessage, 512, _T("0x%8X  %-48s  %-48s  0x%08X    0X%08X   0x%08X  l:%-4ld t:%-4ld r:%-4ld b:%-4ld  %12d  %12d  %13s  %12s  %13s  %13s  %11s\0"),
																		   hWnd,
																		   psClassName,
...
bool SaveCapture(PCAPTURE pCapture, bool bCompress /* = false */)
{
	...
	_tcsncpy_s(psFilename, nLength + 5, pCapture->sFilename, nLength);

	// Check for compression.
	if (bCompress)
	{
		// Append format to string.
		_tcsncpy_s(psFilename + nLength, 5, _T(".zip\0"), 5);
	...
	// Append format to string.
	_tcsncpy_s(psFilename + nLength, 5, _T(".bmp\0"), 5);
...
bool SaveEncodedCapture(PCAPTURE pCapture, EncoderType nEncoder /* = EncoderTypePNG */)
{
	// The following implementation is Windows CE only!
	return false;
	...
	//case EncoderTypeBMP:
	//	sFormat = L"image/bmp";
	//	_tcsncpy_s(psFilename + nLength, nLength + 5, _T(".bmp\0"), 5);
	//	break;
	...
	//	_tcsncpy_s(psFilename + nLength, nLength + 5, _T(".tif\0"), 5);
	//	break;
	//case EncoderTypePNG:
	//default:	
	//	sFormat = L"image/png";
	//	_tcsncpy_s(psFilename + nLength, nLength + 5, _T(".png\0"), 5);
	//	break;
	//}
